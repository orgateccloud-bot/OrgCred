"""
Testes HTTP da régua de cobrança (/cobranca) e da liquidação
(/operacoes/{id}/liquidar) — enforcement de auth/papel e tradução dos
erros de negócio para o envelope unificado {"detail": ..., "codigo": ...}.
"""
...
class TestAutenticacao:
    def test_inadimplencia_sem_token_401(self, client: TestClient) -> None:
        response = client.post(f"/cobranca/operacoes/{uuid.uuid4()}/inadimplencia")
        assert response.status_code == 401
        assert response.json()["codigo"] == "TOKEN_AUSENTE"

    def test_renegociacao_sem_token_401(self, client: TestClient) -> None:
        response = client.post(
            f"/cobranca/operacoes/{uuid.uuid4()}/renegociacao", json=_renegociacao_payload()
        )
        assert response.status_code == 401

    def test_liquidar_sem_token_401(self, client: TestClient) -> None:
        response = client.post(f"/operacoes/{uuid.uuid4()}/liquidar")
        assert response.status_code == 401


class TestInadimplenciaERegularizacao:
    def test_marcar_e_regularizar(
        self,
        ...
        r1 = authed_client.post(f"/cobranca/operacoes/{op_id}/inadimplencia")
        assert r1.status_code == 200
        assert r1.json()["status"] == "inadimplente"

        # inadimplente segue comprometendo o teto
        capital = authed_client.get("/capital/disponivel")
        assert float(capital.json()["disponivel"]) == 20_000.0

        r2 = authed_client.post(f"/cobranca/operacoes/{op_id}/regularizacao")
        assert r2.status_code == 200
        assert r2.json()["status"] == "ativa"

    def test_inadimplencia_de_operacao_registrada_409(
        self,
        authed_client: TestClient,
        db_session: Session,
        tomador_autorizado: uuid.UUID,
        ...
        response = authed_client.post(f"/cobranca/operacoes/{op_id}/inadimplencia")

        assert response.status_code == 409
        assert response.json()["codigo"] == "OC003"

    def test_inadimplencia_inexistente_404(self, authed_client: TestClient) -> None:
        response = authed_client.post(f"/cobranca/operacoes/{uuid.uuid4()}/inadimplencia")
...
class TestRenegociacao:
    def test_renegociacao_ok(
        ...
        response = authed_client.post(
            f"/cobranca/operacoes/{op_id}/renegociacao", json=_renegociacao_payload(35_000)
        )
    ...
    def test_renegociacao_acima_do_teto_422_oc001(
        ...
        response = authed_client.post(
            f"/cobranca/operacoes/{op_id}/renegociacao", json=_renegociacao_payload(60_000)
        )
    ...
    def test_renegociacao_de_proposta_409_oc003(
        ...
        response = authed_client.post(
            f"/cobranca/operacoes/{op_id}/renegociacao", json=_renegociacao_payload(10_000)
        )
    ...
    def test_renegociacao_payload_invalido_422_pydantic(self, authed_client: TestClient) -> None:
        ...
        response = authed_client.post(
            f"/cobranca/operacoes/{uuid.uuid4()}/renegociacao", json=payload
        )

        assert response.status_code == 422


class TestLiquidacaoHTTP:
    def test_liquidar_ok(
        ...
        response = authed_client.post(f"/operacoes/{op_id}/liquidar")
    ...
    def test_liquidar_proposta_409_oc003(
        self,
        authed_client: TestClient,
        db_session: Session,
        tomador_autorizado: uuid.UUID,
    ...
    def test_liquidar_inexistente_404(self, authed_client: TestClient) -> None:
        response = authed_client.post(f"/operacoes/{uuid.uuid4()}/liquidar")
        assert response.status_code == 404
        assert response.json()["codigo"] == "OPERACAO_NAO_ENCONTRADA"
